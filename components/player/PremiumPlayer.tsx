...
export function PremiumPlayer({ mediaId }: { mediaId: string }) {
  const { controller, session, state, isLoading, error } = usePlayerController(mediaId);
  ...
  return (
    <SafeAreaView style={styles.screen} edges={state.isFullscreen ? [] : ['top']}>
      <Animated.View style={[styles.playerShell, state.isFullscreen ? styles.fullscreenShell : styles.portraitShell]}>
        <VideoSurface session={session} state={state} />
